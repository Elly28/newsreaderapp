...
<div class="container mx-auto my-8">
    <div class="p-4">
        <h3 class="text-3xl font-bold text-center" :class="{ 'text-white': darkMode, 'text-black': !darkMode }">Entertainment</h3>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        @foreach($entertainmentArticles as $article)
        <div class="bg-white p-4 shadow-md rounded-lg">
            <img src="{{ $article->urlToImage}}" alt="Image" class="w-full h-48 object-cover rounded-lg mb-4">
            <h2 class="text-xl font-semibold text-gray-800">
                <a href="{{ route('entertainment.show', $article->id) }}" class="hover:text-blue-500">{{ $article->title }}</a>
            </h2>
